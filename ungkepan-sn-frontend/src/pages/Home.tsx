...
import {
  ...
  Storefront, Star, CaretRight, CaretDown, Leaf,
  Heart, Cookie, Coffee, BowlFood, Cake,
  ShoppingCart, ClipboardText, CreditCard, ThumbsUp, Sun,
  MagnifyingGlass
} from '@phosphor-icons/react'
import { getCategories, getFeaturedProducts, getProducts, getTestimonials, submitReview, getSiteContent, resolveImage } from '../api/client'
...
const iconMap: Record<string, React.ElementType> = {
  Heart, ShieldCheck, Package, Clock, Leaf, Cookie, Coffee, BowlFood, Cake, Storefront, ThumbsUp, Sun,
}
...
export default function Home() {
  ...
  const debouncedSearch = (query: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      ...
      setShowSearchDropdown((filteredProducts.length + filteredCategories.length) > 0)
    }, 50)
  }
  ...
  const handleSearchFocus = () => {
    if (heroSearch.trim() && (searchResults.products.length + searchResults.categories.length) > 0) {
      setShowSearchDropdown(true)
    }
  }

  const selectResult = (type: 'product' | 'category', id: string, name: string) => {
    if (type === 'product') {
  ...
  const handleSubmitTestimonial = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const res = await submitReview({ name: form.name, rating: form.rating, review: form.review, product_id: null })
  ...
  const fallbackTestimonials = [
    { id: 'f1', name: 'Andi Wijaya', rating: 5, review: 'Makanan enak banget, bumbunya meresap sampe ke tulang!' },
    { id: 'f2', name: 'Sari Dewi', rating: 5, review: 'Pesan sore, malam udah sampai. Packing rapi, es batu masih beku.' },
    { id: 'f3', name: 'Budi Santoso', rating: 4, review: 'Sudah langganan 1 tahun, kualitas konsisten. Harga bersahabat.' },
    { id: 'f4', name: 'Maya Putri', rating: 5, review: 'Ayam ungkep favoritku, daging empuk banget. Nasi 3 porsi habis sendirian.' },
    { id: 'f5', name: 'Rizky Maulana', rating: 5, review: 'Ikan bakar wangi asap, sambel terasinya nendang. Recommended deh!' },
    { id: 'f6', name: 'Dina Permata', rating: 4, review: 'Delivery cepat, admin ramah. Minta sambel extra diikutin no ribet.' },
    { id: 'f7', name: 'Eko Prasetyo', rating: 5, review: 'Pesen buat arisan kantor, temen-temen pada suka. Bikin ketagihan!' },
    { id: 'f8', name: 'Lina Marlina', rating: 5, review: 'Fresh banget, baru masak kayaknya pas dikirim. Bakal order lagi.' },
    { id: 'f9', name: 'Tommy Gunawan', rating: 4, review: 'Kualitas premium tapi harga masuk kantong. Value for money banget.' },
    { id: 'f10', name: 'Rina Wulandari', rating: 5, review: 'Udah 5x belanja disini, belum pernah nyesel. Consistency is key!' },
  ]

  // Helper: render marquee cards (duplicated for seamless loop)
  const renderMarqueeCards = (data: typeof testimonials, row: 'top' | 'bottom') => {
    const base = [...data]
    // Tambah fallback hanya jika data nyata kurang dari 8 (biar marquee tetap rapat)
    if (base.length < 8) {
      while (base.length < 8) {
        const fb = fallbackTestimonials[base.length % fallbackTestimonials.length]
    ...
    return (
      <div className="bg-white p-3 sm:p-6 rounded-2xl border border-zinc-200 hover:shadow-md transition-shadow flex-shrink-0 w-48 sm:w-80">
        <div className="flex items-center gap-1 mb-2 sm:mb-3">
          {Array.from({ length: 5 }).map((_, j) => (
            <Star
              ...
              className={j < testimonial.rating ? 'text-amber-500' : 'text-zinc-300'}
            />
          ))}
          <span className="text-[10px] sm:text-[11px] text-zinc-400 ml-1">{testimonial.rating}/5</span>
        </div>
        <p className="text-zinc-600 leading-relaxed mb-3 sm:mb-4 text-xs sm:text-base italic">{testimonial.review}</p>
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className="text-xs sm:text-base font-bold text-zinc-900 truncate min-w-0">— {testimonial.name}</span>
        </div>
      </div>
    )
  }
    ...
    <main>
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-6 w-full py-16">
          <div className="max-w-2xl animate-fade-in" style={{ animationDelay: '0.1s' }}>
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold leading-tight mb-4">
              {hero?.headingStart || 'Aneka Olahan Frozen & Minuman'} <br className="hidden md:block" />
              <span className="text-brand-200">{hero?.headingBrand || 'Olahan Ungkepan SN'}</span>
            </h1>
            <p className="text-base md:text-lg text-white/80 leading-relaxed max-w-lg mb-8">
              {hero?.description || 'Enak, bersih, dan dibuat dengan resep rahasia. Homemade berkualitas.'}
            </p>
            ...
            <div className="relative">
              <form onSubmit={handleHeroSearch} className="animate-slide-up w-full md:w-96" style={{ animationDelay: '0.4s' }}>
              ...
              {showSearchDropdown && heroSearch.trim() && (
                <div className="absolute left-0 top-full mt-2 w-full md:w-96 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden z-50 animate-fade-in">
                  {searchResults.products.length > 0 && (
                    <div className="p-2">
                      <p className="px-3 py-1.5 text-xs font-semibold text-white/70 uppercase tracking-wide">Produk</p>
                      {searchResults.products.map((product) => (
                        <button
                          key={product.id}
                          onClick={() => selectResult('product', product.id, product.name)}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 text-left">
                            ...
                            <p className="text-xs text-brand-400 font-bold">Rp {product.price.toLocaleString('id-ID')}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {searchResults.categories.length > 0 && (
                    <div className="p-2 border-t border-white/10">
                      <p className="px-3 py-1.5 text-xs font-semibold text-white/70 uppercase tracking-wide">Kategori</p>
                      {searchResults.categories.map((category) => (
                        <button
                          key={category.id}
                          onClick={() => selectResult('category', category.id, category.name)}
                          className="w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-white/5 transition-colors"
                        >
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-6 py-30">
        <div className="animate-on-scroll">
          <h2 className="text-2xl md:text-3xl font-bold text-zinc-800 mb-2">Kategori</h2>
          ...
          {categories.map((cat, i) => (
            ...
            >
              ...
              <div className="relative z-10 flex items-center justify-center h-full p-4">
                <span className="text-lg md:text-xl font-bold text-zinc-800">{cat.name}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="bg-zinc-50 py-30">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="flex items-end justify-between mb-8 animate-on-scroll">
            <div>
              <h2 className="text-2xl md:text-3xl font-bold text-zinc-800">Produk Favorit</h2>
              <p className="text-zinc-500 mt-1">Yang paling laris di Ungkepan SN</p>
            </div>
            <Link
              to="/products"
            ...
            >
              <div className="relative w-full aspect-[4/3] overflow-hidden flex items-center justify-center p-6">
                ...
                <div className="relative text-center">
                  <h3 className="font-bold text-white text-lg md:text-xl mb-3 leading-tight">Ingin menu lainnya?</h3>
      ...
      <section className="bg-white">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-30">
          <div className="animate-on-scroll">
            ...
            <p className="text-zinc-500 text-center mb-10">Kami beda dari yang lain</p>
          </div>
      ...
      <section className="bg-zinc-50 py-30">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          ...
          <div className="relative grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10 md:gap-y-6">
            ...
            {[
              { icon: Package, title: 'Pilih Produk', desc: 'Jelajahi katalog & pilih lauk favorit kamu' },
              { icon: ShoppingCart, title: 'Tambah ke Keranjang', desc: 'Klik ikon keranjang, atur jumlah pesanan' },
              { icon: ClipboardText, title: 'Checkout', desc: 'Isi data diri, alamat & pilih pengiriman' },
              { icon: CreditCard, title: 'Bayar Pesanan', desc: 'Bayar online atau pilih Bayar di Tempat (COD)' },
            ].map((item, i) => {
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center justify-center w-28 h-28 md:w-32 md:h-32 rounded-full bg-white border-2 border-brand-100 mb-5 group-hover:scale-110 group-hover:border-brand-300 transition-all duration-500">
                    <div className="absolute inset-2 rounded-full bg-gradient-to-br from-brand-50 to-brand-100 flex items-center justify-center">
                      <Icon size={36} weight="fill" className="text-brand-600 group-hover:scale-110 transition-transform duration-500" />
                    </div>
                    {/* Step number badge */}
                  ...
                  <h3 className="font-bold text-zinc-800 text-base md:text-lg mb-1.5">{item.title}</h3>
                  <p className="text-sm text-zinc-500 leading-relaxed max-w-[200px]">{item.desc}</p>
      ...
      <section className="bg-white py-30">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="animate-on-scroll">
            ...
            <p className="text-zinc-500 text-center mb-10">Yang udah cobain pada suka</p>
          </div>
          {testimonials.length === 0 ? (
            <p className="text-center text-zinc-400 animate-on-scroll">Belum ada testimoni</p>
          ) : (
            <div className="relative max-w-6xl mx-auto">
              {/* Top row: Left → Right */}
              <div className="overflow-hidden mb-6">
                <div className="flex gap-6 animate-marquee-left will-change-transform hover:animation-paused" style={{ animationDuration: '30s' }}>
                  {renderMarqueeCards(testimonials, 'top')}
                </div>
              </div>
              {/* Bottom row: Right → Left */}
              <div className="overflow-hidden">
                <div className="flex gap-6 animate-marquee-right will-change-transform hover:animation-paused" style={{ animationDuration: '35s' }}>
      ...
      <section className="bg-white py-30">
        <div className="max-w-7xl mx-auto px-4 md:px-6">
          <div className="animate-on-scroll">
            ...
            <form onSubmit={handleSubmitTestimonial} className="bg-white border border-zinc-200 rounded-2xl p-6 hover:shadow-md transition-shadow space-y-4">
                <h3 className="text-2xl md:text-3xl font-bold text-zinc-800 text-center mb-4">Tulis Testimoni</h3>
                ...
                <div className="space-y-3">
                  <div>
                    <label className="block text-[11px] font-bold text-zinc-500 mb-1">NAMA</label>
                    <input
                      value={form.name}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-zinc-500 mb-1">RATING</label>
                      ...
                      {[1, 2, 3, 4, 5].map((n) => (
                        ...
                        >
                          <Star
                            ...
                            className={n <= form.rating ? 'text-amber-500' : 'text-zinc-300 hover:text-amber-400'}
                          />
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-zinc-500 mb-1">PESAN</label>
                    <textarea
                      value={form.review}
