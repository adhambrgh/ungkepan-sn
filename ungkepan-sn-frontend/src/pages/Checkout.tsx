...
const checkoutSchema = z.object({
  ...
  city: z
    .string()
    .min(1, "Kota wajib diisi/Pilih tersimpan/Pilih langsung di map"),
...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    getSiteContent("shipping")
      ...
      .catch(() => {});
    getSiteContent("shipping_methods")
      .then((data) => {
        if (data)
          setShippingMethods(
            data.filter((m: { is_active: boolean }) => m.is_active),
          );
      })
      .catch(() => {});
  }, []);
  ...
  const shippingMethodList = (() => {
    ...
    if (shippingMethods.length > 0) {
      ...
      methods = [
        ...
        {
          value: "gosend",
          label: "GoSend (Jabodetabek)",
          desc: "Hari ini sampai",
          cost: 0,
          disabled: false,
        },
        {
    ...
    if (shippingConfig) {
      ...
      const cost = !distance
        ? 0
        : Math.round(distance * (shippingConfig.cost_per_km || 0));
  ...
  const onSubmit = async (data: CheckoutForm) => {
    ...
    setSubmitting(true);
    try {
      const payMethod = paymentMethods.find(
        (p) => String(p.id) === data.payment,
      );
      const payLabel = payMethod?.label || data.payment;
      const isCod =
        payMethod?.method === "cod" ||
        /cod|bayar di tempat|cash/i.test(payLabel);
      const isMidtrans =
        !isCod &&
        (payMethod?.method === "midtrans" ||
          /midtrans|snap|online/i.test(payLabel));
      ...
      if (isMidtrans) {
        ...
        try {
          ...
          const openSnap = () => {
            payWithSnap(token, {
              onSuccess: () => {
                ...
                markOrderPaid(orderCode).catch(() => {});
                updateOrderStatus(orderCode, "processed");
                recordOrder();
                clearCart();
                setPendingSnapToken(null);
                setSubmitted(true);
              ...
              onClose: () => {
                ...
                fetch(
                  `${import.meta.env.VITE_API_URL || ""}/api/orders-snap-token.php`,
                  {
                    method: "DELETE",
          ...
          setMidtransLoading(false);
          const message =
            e instanceof Error ? e.message : "Popup pembayaran belum siap";
        ...
        <div className="bg-white rounded-2xl border-2 border-dashed border-zinc-200 p-6 mb-6 max-w-md mx-auto font-mono text-sm">
          ...
          <div className="border-t-2 border-zinc-800 pt-2 flex justify-between font-bold text-zinc-800">
            ...
            <span>
              Rp{" "}
              {(
                lastOrder!.total + (lastOrder!.shippingCost || 0)
          ...
          <div className="mt-3 pt-3 border-t border-dashed border-zinc-200 text-xs text-zinc-500 space-y-0.5">
            ...
            {lastOrder!.shipping === "lokal" && (
              <p>Jarak: {distance?.toFixed(1)} km</p>
            )}
            <p>
              Alamat: {lastOrder!.address}, {lastOrder!.city}
            </p>
          </div>
        </div>
        ...
        <div className="max-w-md mx-auto">
          {isCod ? (
            ...
            <div className="bg-brand-50 border border-brand-100 rounded-xl p-4 text-center mb-4">
              ...
              <p className="text-xs text-zinc-500 mt-1">
                Pembayaran diproses lewat popup Midtrans. Lanjutkan sesuai
                metode yang kamu pilih.
              </p>
            </div>
          )}
        </div>
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6 items-start">
          <div className="space-y-6">
                    ...
                    >
                      {selectedAddress ? (
                        <span className="text-zinc-800 min-w-0">
                          ...
                          <span className="block text-zinc-500 text-xs mt-0.5 leading-snug">
                      ...
                      <CaretDown
                        size={16}
                        className={`text-zinc-400 shrink-0 transition-transform ${addrOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    {addrOpen && (
                        ...
                        {savedAddresses.map((a) => (
                          ...
                          >
                            <div className="flex items-center gap-2">
                              ...
                              {a.is_default && (
                                <span className="text-[10px] font-bold text-brand-600 bg-brand-50 px-1.5 py-0.5 rounded-md uppercase">
                                  Utama
                                </span>
                              )}
                              {a.name && (
                            ...
                            <span className="block text-xs text-zinc-600 mt-1 leading-relaxed">
                              {formatAddress(a)}
                            </span>
                          </button>
                        ))}
            ...
            <div className="bg-white rounded-2xl p-6 sm:p-8">
              <SectionHeading
                icon={<Truck size={17} weight="fill" />}
                      ...
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {!s.disabled &&
                            ...
                            s.value === "lokal" &&
                            distance && (
                              <span className="font-bold text-sm text-green-600 shrink-0">
                                GRATIS
                              </span>
                            )}
                        </div>
                        <p
                          className={`mt-1 text-xs leading-relaxed ${s.disabled ? "text-red-500" : "text-zinc-500"}`}
                        >
                          {s.desc}
                        </p>
                      </div>
          ...
          <aside className="lg:sticky lg:top-28 bg-white rounded-2xl p-6 sm:p-7">
            ...
            <div className="mt-5 pt-5 border-t border-dashed border-black/10 space-y-2.5 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`font-semibold ${selectedShippingCost > 0 ? "text-zinc-800" : "text-green-600"}`}
                >
            ...
            <p className="mt-3 text-xs text-center text-zinc-400">
              Dengan membuat pesanan, kamu setuju dengan syarat & ketentuan
              Ungkepan SN.
            </p>
          </aside>
        </div>
        ...
        >
          ...
          >
            ...
            <p className="text-center text-sm text-zinc-500 mt-4">
              Scan menggunakan aplikasi pembayaran (GoPay, OVO, DANA, Mobile
              Banking, dll)
            </p>
          </div>
        </div>
      ...
      {midtransLoading && (
        <div className="fixed inset-0 z-[60] bg-white/90 flex flex-col items-center justify-center p-6">
          ...
          <p className="text-sm text-zinc-400 text-center mt-1">
            Popup pembayaran akan terbuka. Jika tidak muncul, pastikan pop-up
            diizinkan di browser.
          </p>
        </div>
      )}
    </main>
